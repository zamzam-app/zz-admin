...
import { getOutlets } from "../lib/services/api/outlet.api";
...
const Infrastructure: React.FC = () => {
  ...
  const handleOpenAdd = () => {
    setNewOutlet({ name: "", description: "", type: "", managerId: "", addressId: "" });
    setIsModalOpen(true);
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    message.success("Form submitted (API call pending)");
      ...
      {loading ? (
        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6'>
          {outlets.map((outlet) => (
            <Card
              key={outlet._id}
              className='p-6 border border-gray-100 rounded-[28px] hover:border-[#D4AF37] transition-all shadow-sm'
            >
              <div className='flex items-start justify-between mb-6'>
                <div className='flex items-center gap-4'>
                  <div className='w-12 h-12 rounded-2xl bg-[#1F2937] text-[#D4AF37] flex items-center justify-center'>
                  ...
                  <div>
                    <h4 className='font-black text-lg text-[#1F2937]'>{outlet.name}</h4>
                    <p className='text-[10px] text-gray-400 uppercase font-bold tracking-widest'>
                      {outlet.type || 'General Outlet'}
                    </p>
                  </div>
                </div>
              ...
              <div className='space-y-3 text-sm text-gray-600'>
                ...
                <div className='flex items-center gap-2'>
                  <User size={14} />
                  Manager ID: {outlet.managerId || 'Unassigned'}
                </div>
                <div className='flex items-center gap-2'>
                  <Captions size={14} />
                  Status: <span className={outlet.isActive ? "text-green-600" : "text-red-500"}>
              ...
              <div className='flex gap-3 mt-6'>
                ...
                <button className='flex-1 py-3 bg-[#1F2937] text-white rounded-xl hover:bg-gray-800 font-bold'>
                  Edit
                </button>
              </div>
            </Card>
        ...
        <form onSubmit={handleSubmit} className='flex flex-col gap-8'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-8'>
            ...
            <Input
                ...
                onChange={(e) => setNewOutlet({ ...newOutlet, managerId: e.target.value })}
            />
          ...
          <div className='flex justify-end gap-4 pt-6 border-t border-gray-100'>
            <Button variant='ghost' onClick={() => setIsModalOpen(false)} className='font-bold text-gray-400'>
              Cancel
            </Button>
            <Button type='submit' variant='admin-primary' className='px-10 py-3.5 rounded-2xl font-black'>
              Save Outlet
            </Button>
          </div>
        </form>
